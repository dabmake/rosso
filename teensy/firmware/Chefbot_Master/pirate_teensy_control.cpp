 ...
 *
 * @author Murat Calis
 * @copyright 2016 Murat Calis, mc@pirate-robotics.net
 * @date 25.07.2015, 02.04.2016, 04.05.2016, 25.09.2016
 * @name pirate_teensy_control.cpp
 * @description (Arduino Nano) Teensy Controller for the IR, PIR, RELAY and MD25 Motors
 * 01.10.2016: Switched to Teensy because Nano has only 2K RAM but Teensy has 64K like C64!
 *
 */
...
#define MODE								15			// MD25 Operation Mode Register. We use Mode-Value 1: motors -127 +128

// ODOMETRY
#define MAX_CMD_VELOCITY 1.1				// Velocity in m/s
#define MAX_MD25_SPEED 30						// Speed in Byte-range of MD25 speed register
...
// Odometry Setup
float wheels_track = 0.24;					// Track = distance between both wheels starting from the middle of rubber tread (26mm).
...
void loop()
{
		...
		if (irrecv.decode(&results))
		{
			...
			// 3191976270 = VOL+
			// 3191950770 = Power ON
			// 3191967090 = Power OFF
			//
			// PHILIPS CD
			// 2534555274 = Power ON
			// 2488819018 = Power OFF
			// ***********************
...
long encoder1(){                                       		// Function to read and display value of encoder 1 as a long
  ...
  position = Wire.read();				// First byte for encoder 1, HH.
  position <<= 8;
...
// Get MD25 Voltage
const int volts(){                                          // Function to read and display battery volts as a single byte
